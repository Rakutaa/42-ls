...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/26 15:37:22 by vtran             #+#    #+#             */
/*   Updated: 2020/01/31 15:41:18 by vtran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			*long_db(long double db, int prec)
{
	...
	if (i > 0 && init == 0)
		...
		tmp = ft_strjoin_f(tmp, ft_itoa((long long)db));
	return (tmp);
}
...
static char			*zero_prec(long double nb, t_print **p)
{
	...
	r = nb < 0 && nb > -1 ? ft_strjoin_f(ft_strdup("-"), r) : r;
	if ((*p)->flags & F_HASH)
		r = ft_strjoin_f(r, ft_strdup("."));
	return (r);
}
...
char				*ftoa(long double nb, int prec, t_print **p)
{
	...
	tmpdb = dcml;
	r = ft_strjoin_f(ft_itoa(ints), ft_strdup("."));
	r = ft_strjoin_f(r, ft_memset(ft_strnew(zp(nb, prec)), '0', zp(nb, prec)));
	dcml *= ft_pow(10, prec);
	dcml += rounding(prec, tmpdb);
	if (ft_count_digits_l((int64_t)dcml, 10) > prec)
	{
		ints = ft_atoi(r) > 0 ? ft_atoi(r) + 1 : ft_atoi(r) - 1;
		r = ft_strjoin_f(ft_itoa(ints), ft_strdup("."));
		r = ft_strjoin_f(r, ft_memset(ft_strnew(prec), '0', prec));
	}
	else
		r = prec > 18 ? ft_strjoin_f(r, long_db(tmpdb, prec))
		: ft_strjoin_f(r, ft_itoa((uint64_t)dcml));
	r = nb < 0 && nb > -1 ? ft_strjoin_f(ft_strdup("-"), r) : r;
	return (r);
}
