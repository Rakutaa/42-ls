...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/26 15:37:06 by vtran             #+#    #+#             */
/*   Updated: 2020/01/31 15:36:33 by vtran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*precision(t_print **p, char *str)
{
	...
	if (len > 0 && (*p)->c == 'o' && (*p)->flags & F_HASH &&\
		check_if_zeros(str))
		str = ft_strjoin_f(ft_memset(ft_strnew(len), '0', len - 1), str);
	else
		str = ft_strjoin_f(ft_memset(ft_strnew(len), '0', len), str);
	if (flag)
		str = ft_strjoin_f(ft_strdup("-"), str);
	return (str);
}
...
char		*space(t_print **p, char *str)
{
	...
	if (flag == 0 && ((*p)->pad <= (int)ft_strlen(str) ||\
		...
		return (ft_strjoin_f(ft_strdup(" "), str));
	return (str);
}
...
char		*ox_prefix(t_print **p, char *str)
{
	...
	{
		if ((*p)->c == 'x')
			str = ft_strjoin_f(ft_strdup("0x"), str);
		else if ((*p)->c == 'X')
			str = ft_strjoin_f(ft_strdup("0X"), str);
		else if ((*p)->c == 'o')
			str = ft_strjoin_f(ft_strdup("0"), str);
	}
	if ((*p)->c == 'X')
